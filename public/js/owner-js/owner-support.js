        ...
        function loadContacts() {
            try {
                ...
                var contacts = JSON.parse(localStorage.getItem('mangment_connect_links')) || [];

                if (contacts.length === 0) {
                    container.innerHTML = '<p style="color: var(--text-muted); font-size: 0.85rem; text-align: center; padding: 20px;">لا توجد أرقام تواصل متاحة حالياً. سيتم إضافتها من لوحة الإدارة.</p>';
                    return;
                }
                ...
                for (var i = 0; i < contacts.length; i++) {
                    ...
                    html += '<a href="' + waLink + '" target="_blank" class="contact-item">' +
                        '<div class="contact-icon-emoji">💬</div>' +
                        '<div class="contact-info">' +
                        '<span class="label">' + escapeHtml(c.role || 'الدعم الفني') + '</span>' +
        ...
        function sendMessage(e) {
            ...
            try {
                ...
                // Save to owner messages
                var messages = JSON.parse(localStorage.getItem('owner_support_messages')) || [];
                messages.push(message);
                localStorage.setItem('owner_support_messages', JSON.stringify(messages));

                // Add to management unified queue
                ...
                localStorage.setItem('management_support_messages', JSON.stringify(queue));

                if (subjectEl) subjectEl.value = '';
                if (bodyEl) bodyEl.value = '';

                renderMessages();
                showToast('success', '✅ تم إرسال رسالتك بنجاح! سيتم الرد عليك قريباً');
            } catch (e) {
