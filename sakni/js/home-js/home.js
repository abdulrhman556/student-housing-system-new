...
function renderProperties() {
    ...
    // ✅ التعديل الرئيسي: قراءة العقارات المنشورة من المفتاح الصحيح
    const publishedProperties = JSON.parse(localStorage.getItem("mangment_flutter_properties")) || [];

    if (publishedProperties.length > 0) {
        activePropertiesList = publishedProperties.map(p => ({
            id: p.id,
            title: p.title || p.name || 'عقار بدون اسم',
            price: parseInt(p.price) || 0,
            bedsLeft: parseInt(p.bedsLeft) || parseInt(p.beds) || 1,
            distance: p.distance || 2.5,
            region: p.region || p.area || 'بني سويف',
            gender: p.gender || 'male',
            services: p.services || p.amenities || [],
            img: p.img || (p.images && p.images.length > 0 ? (p.images[0].base64 || p.images[0]) : 'https://images.unsplash.com/photo-1554995207-c18c203602cb?q=80&w=500'),
            _fullData: p
        }));
    }
...
function loadChatContacts() {
    ...
    if (phone) {
        const cleanPhone = phone.replace(/[^0-9]/g, '');
        contactsList.innerHTML = `<a href="https://wa.me/${cleanPhone}" target="_blank" class="contact-chip">💬 الدعم الفني</a>`;
    } else {
...
function sendChatMessage() {
    ...
    if (!phone) {
        alert('لا يوجد رقم دعم متاح حالياً');
        return;
    }

    const cleanPhone = phone.replace(/[^0-9]/g, '');
    const waText = text ? '?text=' + encodeURIComponent(text) : '';
    window.open('https://wa.me/' + cleanPhone + waText, '_blank');
...
// ===== SUPPORT PHONE FROM MANGMENT-XX =====
function showSupportPhone(e) {
